...
它是交互式架构图，适合演示角色视图、注册 / 登录 / 搜索 / 运维 / 备份恢复流程。V1.8 起，`architecture.html` 支持演示模式、流程播放和 SVG 导出，适合交付讲解和团队培训。架构相关行为变化时，必须同步更新页面内节点、连线和讲解内容。本文负责更详细的文字解释，静态 SVG 负责一页式归档，`architecture.html` 负责现场讲解。

V1.8 起，`architecture.html` 也支持：

- 演示模式：弱化左右信息区，突出主 SVG 架构图，适合投屏讲解。
- 流程播放：按注册、登录、搜索、启动监控、备份恢复的步骤自动高亮节点，并同步右侧详情。
- SVG 导出：把当前高亮状态导出为可读的 SVG 文件，适合交付材料、培训课件和离线归档。

维护提示：接口、启动方式、存储结构或页面入口变化时，要同步更新 `architecture.html` 和架构说明，避免交付讲解与实际系统脱节。
...
V1.8 起，环境变量解析、默认值和启动前配置校验集中在 `app_config.py`。`main.py` 仍保留运行时常量名，目的是减少路由层改动并保持接口行为稳定。

### `app_config.py`
负责：
- 环境变量解析
- 默认值管理
- CPU/GPU、日志、CORS、图片限制、活体、维护模式等配置汇总
- 启动前配置校验，例如 production 环境必须配置 API Key

### `api_schemas.py`
负责：
- 请求模型
- 响应模型
- OpenAPI schema 使用的字段说明和示例

路由仍在 `main.py` 中编排，避免一次性拆分路由造成行为回归。
...
- 备份路径校验
- 数据库备份文件创建
- 数据库恢复文件复制和 stale WAL/SHM 清理

`main.py` 只负责 API 鉴权、请求解析和调用这些 helper。
...
### 11.4 认证能力仍然较轻
当前已有可配置的轻量活体 challenge，但它仍然不是完整风控平台。当前仍然没有：

- 完整防重放体系
- 更强的集中化风控层
- 独立账号、权限、token / session 签发体系
