...
- [x] 真实用户正常完成 face login 时不需要额外复杂动作。
- [x] 明显静态照片或静态屏幕样例能被标记为更高风险。
- [x] 高风险样例不会被静默当作普通活体成功。
- [x] 活体失败或高风险时返回简短中文原因。
- [x] audit 能记录活体风险等级、主要原因和 terminal 信息。
- [x] 现场验收报告能列出真人、打印照片、手机屏幕、电脑屏幕、播放视频样例的结果。
- [x] 文档明确说明 V2.1 是轻量防翻拍增强，不是企业级强活体。
- [x] 不破坏 V2.0 business-demo 和终端 demo 的现有登录链路。
