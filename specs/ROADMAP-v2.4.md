...
- [x] Face API 侧联动验收模板已创建，并能指导一次完整联动验收。
- [x] WMS 侧终端 runbook 已创建，并能指导摄像头、登录、日志和证据回收。
- [x] 文档入口、季度计划和 specs 索引能指向 V2.4。
- [x] 实施任务明确区分 Face API 仓库和 WMS 仓库，不混用提交范围。
- [x] 验收模板明确不保存原图、视频帧、embedding 或 API Key。
- [x] 验收模板明确问题归因必须落到算法底座、终端采集或业务流程。
- [x] `git diff --check` 通过。
- [x] 若修改 WMS 仓库，WMS 工作区变更已单独检查，提交命令与 Face API 仓库分开处理。
